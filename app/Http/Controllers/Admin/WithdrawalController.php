...
class WithdrawalController extends Controller
{
    public function pending()
    {
        ...
        $withdrawals = Withdrawal::pending()->with(['owner','method'])->orderBy('id','desc')->paginate(getPaginate());
    ...
    public function approved()
    {
        ...
        $withdrawals = Withdrawal::approved()->with(['owner','method'])->orderBy('id','desc')->paginate(getPaginate());
        $emptyMessage = 'No withdrawal found';
        return view('admin.withdraw.withdrawals', compact('pageTitle', 'withdrawals', 'emptyMessage'));
    }
    ...
    public function rejected()
    {
        ...
        $withdrawals = Withdrawal::rejected()->with(['owner','method'])->orderBy('id','desc')->paginate(getPaginate());
        $emptyMessage = 'No withdrawal found';
        return view('admin.withdraw.withdrawals', compact('pageTitle', 'withdrawals', 'emptyMessage'));
    }
    ...
    public function logViaMethod($methodId,$type = null){
        ...
        if ($type == 'approved') {
            ...
            $withdrawals = Withdrawal::where('status', 1)->with(['owner','method'])->where('method_id',$method->id)->orderBy('id','desc')->paginate(getPaginate());
        }elseif($type == 'rejected'){
            $pageTitle = 'Rejected Withdrawals Via '.$method->name;
            $withdrawals = Withdrawal::where('status', 3)->with(['owner','method'])->where('method_id',$method->id)->orderBy('id','desc')->paginate(getPaginate());

        }elseif($type == 'pending'){
            $pageTitle = 'Pending Withdrawals Via '.$method->name;
            $withdrawals = Withdrawal::where('status', 2)->with(['owner','method'])->where('method_id',$method->id)->orderBy('id','desc')->paginate(getPaginate());
        }else{
            $pageTitle = 'Withdrawals Via '.$method->name;
            $withdrawals = Withdrawal::where('status', '!=', 0)->with(['owner','method'])->where('method_id',$method->id)->orderBy('id','desc')->paginate(getPaginate());
        }
        $emptyMessage = 'No withdrawal found';
    ...
    public function dateSearch(Request $request,$scope){
        ...
        $withdrawals = $withdrawals->with(['owner', 'method'])->paginate(getPaginate());
    ...
    public function details($id)
    {
        ...
        $withdrawal = Withdrawal::where('id',$id)->where('status', '!=', 0)->with(['owner','method'])->firstOrFail();
        $pageTitle = $withdrawal->owner->username.' Withdraw Requested ' . showAmount($withdrawal->amount) . ' '.$general->cur_text;
    ...
    public function approve(Request $request)
    {
        ...
        $withdraw = Withdrawal::where('id',$request->id)->where('status',2)->with('owner')->firstOrFail();
        $withdraw->status = 1;
    ...
    public function reject(Request $request)
    {
        ...
        $owner = Owner::find($withdraw->owner_id);
